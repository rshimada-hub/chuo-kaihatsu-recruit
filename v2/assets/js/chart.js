...
(function () {
  ...
  [5, 10, 15, 20].forEach(function (g) {
    var yy = Y(g);
    s += '<line x1="' + L + '" y1="' + yy + '" x2="' + Rr + '" y2="' + yy + '" stroke="var(--ck-border)" stroke-width="1"/>';
    s += '<text x="' + (L - 8) + '" y="' + (yy + 4) + '" text-anchor="end" font-family="Cormorant Garamond" font-size="11" fill="var(--ck-fg-subtle)">' + g + '</text>';
  });
  var cum = [0, 0, 0, 0, 0];
  ...
  // 軸
  s += '<line x1="' + L + '" y1="' + B + '" x2="' + (Rr + 6) + '" y2="' + B + '" stroke="var(--ck-fg-subtle)" stroke-width="1.5"/>';
  s += '<line x1="' + L + '" y1="' + B + '" x2="' + L + '" y2="' + (T - 8) + '" stroke="var(--ck-fg-subtle)" stroke-width="1.5"/>';
  s += '<path d="M' + L + ' ' + (T - 14) + ' l-4 8 h8 z" fill="var(--ck-fg-subtle)"/>';
  s += '<text x="' + (L - 6) + '" y="' + (T - 20) + '" font-family="Noto Sans JP" font-weight="700" font-size="13" fill="var(--ck-fg-muted)">売上高（億円）</text>';
  years.forEach(function (yr) {
    s += '<text x="' + X(yr) + '" y="' + (B + 22) + '" text-anchor="middle" font-family="Cormorant Garamond" font-weight="600" font-size="12" fill="var(--ck-fg-subtle)">' + yr + '</text>';
  });
  s += '<text x="' + X(2026) + '" y="' + (B + 40) + '" text-anchor="middle" font-family="Cormorant Garamond" font-weight="700" font-size="10.5" fill="var(--ck-accent-text)">現在</text>';
  s += '<text x="' + X(2035) + '" y="' + (B + 40) + '" text-anchor="middle" font-family="Cormorant Garamond" font-weight="700" font-size="10.5" fill="var(--ck-accent-text)">目標</text>';
  s += '<text x="' + (X(2026) + 6) + '" y="' + (Y(6) - 8) + '" font-family="Cormorant Garamond" font-weight="800" font-size="13" fill="var(--ck-fg)">6.0</text>';
  s += '<text x="' + X(2035) + '" y="' + (Y(20) - 8) + '" text-anchor="end" font-family="Cormorant Garamond" font-weight="800" font-size="16" fill="var(--ck-accent-text)">20</text>';
